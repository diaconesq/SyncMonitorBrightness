...
namespace SyncMonitorBrightness
{
    ...
    public class PhysicalMonitorBrightnessController : IDisposable
	{
		...
		private IList<MonitorInfo> Monitors { get; set; }
		...
		private void UpdateMonitorsCurrentValue()
		{
			var toRemove = new List<MonitorInfo>();
			foreach (var monitor in Monitors)
			{
				uint minValue = 0, currentValue = 0, maxValue = 0;
				if (!GetMonitorBrightness(monitor.Handle, ref minValue, ref currentValue, ref maxValue))
				{
